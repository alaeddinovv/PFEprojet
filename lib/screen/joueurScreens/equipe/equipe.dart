...
class _EquipeState extends State<Equipe> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          Expanded(
            ...
            Padding(
              ...
              child: BlocConsumer<EquipeCubit, EquipeState>(
                ...
                builder: (context, state) {
                  if (state is GetEquipeImInLoading && EquipeCubit.get(context).cursorId1 == '') {
                    return const Center(child: CircularProgressIndicator());
                  }
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      return _buildEquipeImInItem(
                          EquipeCubit.get(context).equipeImInData[index],
                          index,
                          context);
                    },
                    separatorBuilder: (context, int index) => const SizedBox(height: 16),
                    itemCount: EquipeCubit.get(context).equipeImInData.length,
                    shrinkWrap: true, // to prevent infinite height error
                  );
                },
              ),
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      return _buildEquipeInvitationItem(
                          EquipeCubit.get(context).equipeInviteData[index],
                          index,
                          context);
                    },
                    separatorBuilder: (context, int index) => const SizedBox(height: 16),
                    itemCount: EquipeCubit.get(context).equipeInviteData.length,
                    shrinkWrap: true, // to prevent infinite height error
                  );
                },
              ),
      ...
      floatingActionButton: isSelected[0] ? FloatingActionButton(
        onPressed: () {
...
//---------------------------- my team----------------------------------------------------------
  Widget _buildEquipeItem(EquipeData model, int index, BuildContext context) {
    return InkWell(
      onTap: () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => MyEquipeDetailsScreen(equipeData: model)),
        );
      },
          ...
          child: Row(
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: () {
                  navigatAndReturn(context: context, page: EditEquipe(equipeModel: model));
  ...
  Widget _buildAllEquipeItem(EquipeData model, int index, BuildContext context) {
    return InkWell(
      onTap: () {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => AllEquipeDetailsScreen(equipes: model)),
        );
      },
      splashColor: Colors.transparent,  // Prevents ripple effect on tap
  ...
  Widget _buildEquipeImInItem(EquipeData model, int index, BuildContext context) {
    return InkWell(
      onTap: () {
        // Add your onTap functionality here if needed
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => EquipeImInDetailsScreen(equipeImInData: model)),
        );
      },
      splashColor: Colors.transparent,  // Prevents ripple effect on tap
      highlightColor: Colors.transparent, // Prevents highlight effect on tap
              ...
              Row(
                ...
                children: [
                  ...
                  ElevatedButton(
                      onPressed: () {
                        ...
                        EquipeCubit.get(context).accepterInvitation(id: model.id).then((_) {
                          ...
                          showToast(msg: "Failed to accept invitation", state: ToastStates.error);
                        });
                      },
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      EquipeCubit.get(context).refuserInvitation(id: model.id).then((_) {
                        showToast(msg: "Invitation refused", state: ToastStates.success);
                        EquipeCubit.get(context).getEquipeInvite(); // Refresh the list after refusing
                      }).catchError((error) {
                        showToast(msg: "Failed to refuse invitation", state: ToastStates.error);
                      });
                    },
                    iconSize: 24,
